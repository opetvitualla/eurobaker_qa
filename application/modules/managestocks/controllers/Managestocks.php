...
class Managestocks extends MY_Controller {
	...
	public function save_stock_transfer() {
		...
		if(!empty($transfer)){
			...
			$data = array(
				...
				"counterchecked_by"	=> $transfer["counterchecked_by"]
			);
			...
			if ($transfer_id) {
				...
				$this->update_inventory_from_outlet($items, $transfer_id); //update inventory from origin
	...
	private function get_receiver_user ($id){
		...
		$receiver = (!empty($getdata) ? $getdata[0]:'');
		return $receiver;
	...
	private function update_inventory_from_outlet($items = array(), $trans_id= 0){
			$origin_branch = _get_branch_assigned();

			if(!empty($items)){
				foreach ($items as $item) {

					$par['select'] = 'quantity';
					$par['where']  = array( 'FK_raw_material_id' => $item->item_id, 'FK_outlet_id' => $origin_branch );
